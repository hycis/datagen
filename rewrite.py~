...
def start(file_num, cropSize):
    ...
    f = open(path_text+file_num+'.txt', 'r')
    string = f.read().split()
    ...
    for num in range(len(string)):
        ...
        image = imread(path_voc+'JPEGImages/'+filename)
        tree = ET.parse(path_voc+'Annotations/'+filename.split('.')[0] + '.xml')
        root = tree.getroot()
        label_bndbox = []
        label_class = []
